...
_DEFAULT_TEMPLATE = Label("//deps/rules_pkg_config:pkg_config.BUILD.tpl")
...
def _run_pkg_config(repository_ctx, command_line):
    """Run command_line with system PKG_CONFIG_PATH and return its
    tokenized output."""
...
def setup_pkg_config_repository(repository_ctx):
    ...
    tool_path = _find_binary(repository_ctx, "pkg-config")
    if not tool_path:
        return struct(error = "Could not find pkg-config")
